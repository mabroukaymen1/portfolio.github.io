...
const technologies = [
  ...
  { name: 'Solenoid Valves', category: 'Actuator' },
  { name: 'Stepper Motor', category: 'Actuator' },
  { name: 'Limit Switches', category: 'Sensor' },
  { name: 'C++ / Arduino', category: 'Firmware' },
  { name: 'Real-time DB', category: 'Database' },
];

const AquaTechTechStack: React.FC = () => {
        ...
        >
          {technologies.map((tech, idx) => (
            <motion.div 
              key={idx}
              variants={fadeInUp}
              className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 text-center hover:border-cyan-500 hover:shadow-md transition-all group"
            >
              <div className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 mb-1">{tech.category}</div>
              <div className="font-bold dark:text-white group-hover:text-cyan-500 transition-colors uppercase tracking-tight">{tech.name}</div>
            </motion.div>
          ))}
        </motion.div>
      </div>
...
export default AquaTechTechStack;
