...
const technologies = [
  { name: 'VOSK ASR', category: 'Speech Recognition' },
  { name: 'Gemini AI', category: 'NLU' },
  { name: 'Flutter', category: 'Mobile App' },
  { name: 'Firebase', category: 'Backend' },
  { name: 'TCP/IP', category: 'Communication' },
  { name: 'REST API', category: 'API' },
  { name: 'WebSockets', category: 'Real-time' },
  { name: 'Python', category: 'Language' },
];

const BeemoTechStack: React.FC = () => {
        ...
        >
          {technologies.map((tech, idx) => (
            <motion.div 
              key={idx}
              variants={fadeInUp}
              className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 text-center hover:border-purple-500 hover:shadow-md transition-all group"
            >
              <div className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 mb-1">{tech.category}</div>
              <div className="font-bold dark:text-white group-hover:text-purple-500 transition-colors uppercase tracking-tight">{tech.name}</div>
            </motion.div>
          ))}
        </motion.div>
      </div>
